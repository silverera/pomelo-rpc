...
MailBox.prototype.connect = function(tracer, cb) {
  ...
  logger.debug('rpc client %s connect to remote server %s host: %s port: %s',
      self.serverId, self.id, self.host, self.port);
  this.socket.connect({
    clientId: 'MQTT_RPC_' + self.serverId + '_' + Date.now()
  }, function() {
    if (self.connected) {
      return;
    }
  ...
  this.socket.on('error', function(err) {
    logger.error('rpc socket %s is error, remote server %s host: %s, port: %s error: %j', self.serverId, self.id, self.host, self.port, err.stack);
    self.emit('close', self.id);
  });
...
var setCbTimeout = function(mailbox, id, tracer, cb) {
  var timer = setTimeout(function() {
    ...
    var eMsg = util.format('rpc %s callback timeout %d, remote server %s host: %s, port: %s', mailbox.serverId, mailbox.timeoutValue, mailbox.id, mailbox.host, mailbox.port);
    logger.error(eMsg);
...
var clearCbTimeout = function(mailbox, id) {
  if (!mailbox.timeout[id]) {
    logger.debug('timer is not exsits, serverId: %s remote: %s, host: %s, port: %s', mailbox.serverId, mailbox.id, mailbox.host, mailbox.port);
    return;
  }
